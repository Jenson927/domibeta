...
import { useDeductReasonsStore } from '@/stores'
...
const newPoints = ref(50)
const showAddForm = ref(false)

const DEDUCT_CATEGORIES = ['学习', '行为', '生活', '品德', '运动', '其他']

const categoryOptions = computed(() => {
  const existingCats = deductReasonsStore.categories
  const allCats = new Set([...existingCats, ...DEDUCT_CATEGORIES])
...
function startEdit(reason: DeductReason) {
...
function saveEdit() {
  if (!newName.value) return
  deductReasonsStore.editReason(editingId.value, {
    name: newName.value,
    icon: newIcon.value,
    category: newCategory.value,
    points: newPoints.value
...
function removeReason(id: number) {
  deductReasonsStore.removeReason(id)
}
</script>

<template>
  <AppModal v-model="isOpen" title="扣分原因管理">
    <!-- List -->
    <div class="mb-4">
      <div
        v-for="reason in deductReasonsStore.allDeductReasons"
        :key="reason.id"
      ...
      >
        ...
        <template v-else>
          <span>{{ reason.icon }} {{ reason.name }} <span class="text-[#F44336] text-[12px] font-bold">-{{ reason.points }}分</span> <span class="text-[#999] text-[12px]">({{ reason.category }})</span></span>
    ...
    <div v-else class="bg-[#FFF3E0] rounded-[10px] p-4">
      ...
      <div class="flex items-center gap-2">
        ...
        <button class="p-2 bg-[#F44336] text-white rounded-[8px] cursor-pointer" @click="addReason">添加</button>
      </div>
    </div>
  </AppModal>
</template>
